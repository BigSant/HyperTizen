        ...
        public async Task Start()
        {
            try
            {
                ...
                if (Globals.Instance.DiagnosticMode)
                {
                    ...
                    // SCAN FOR ALTERNATIVES — runs in background Task so WebSocket stays alive
                    if (SDK.SystemInfo.TizenVersionMajor >= 8)
                    {
                        Helper.Log.Write(Helper.eLogType.Info, "Scanning for alternative capture methods (background)...");
                        _ = Task.Run(() =>
                        {
                            try
                            {
                ...
                const int maxBlackFramesBeforeFallback = 30; // ~2s at 15fps
                ...
                while (!_cancellationTokenSource.Token.IsCancellationRequested && Globals.Instance.Enabled)
                {
                    try
                    {
                        ...
                        if(isConnected)
                        {
                            ...
                            // Capture directly — HyperionClient already runs on a background thread,
                            // so no Task.Run needed here (avoids thread-pool competition with sender).
                            CaptureResult captureResult = _selectedCaptureMethod.Capture(
                            ...
                            {
                                // Hand off frame to dedicated sender thread — returns immediately, never blocks.
                                Networking.PostImage(
                                    captureResult.YData,
                                    captureResult.UVData,
                                    captureResult.Width,
                                    captureResult.Height);

                                int captureMs = (int)watchFPS.Elapsed.TotalMilliseconds;

                                // Throttle to 15 FPS — sleep remaining time to fill 66 ms frame budget
                                const int targetFrameMs = 1000 / 15; // 66 ms
                                int sleepMs = targetFrameMs - captureMs;
                                if (sleepMs > 0)
                                    await Task.Delay(sleepMs, _cancellationTokenSource.Token);

                                // Measure TOTAL frame time (capture + sleep) for accurate FPS
                                watchFPS.Stop();
                                var elapsedFPS = 1.0 / watchFPS.Elapsed.TotalSeconds;
                            ...
                            else
                            {
                                ...
                                bool isBlackFrame = errorMsg.Contains("Black frame");
                                ...
                                {
                                    Helper.Log.Write(Helper.eLogType.Warning, $"Capture failed: {errorMsg}");
                                    consecutiveErrors++;
                                    consecutiveBlackFrames = 0;
                                ...
                                bool shouldFallback =
                                    consecutiveBlackFrames >= maxBlackFramesBeforeFallback ||
                                    consecutiveErrors >= maxConsecutiveErrors;
                                if (shouldFallback)
                                {
                                    string reason = consecutiveBlackFrames >= maxBlackFramesBeforeFallback
                                        ? $"{consecutiveBlackFrames} black frames"
                                        : $"{consecutiveErrors} capture errors";
                                    Helper.Log.Write(Helper.eLogType.Warning,
                                        $"[HyperionClient] {reason}; advancing capture fallback chain");
                                    _selectedCaptureMethod = _captureSelector?.SelectNextMethod(
                                        _selectedCaptureMethod);
                                    consecutiveBlackFrames = 0;
                                    consecutiveErrors = 0;
                                    if (_selectedCaptureMethod == null)
                                    {
                                        Helper.Log.Write(Helper.eLogType.Error,
                                            "[HyperionClient] No capture fallback remains");
                                        await Task.Delay(2000, _cancellationTokenSource.Token);
                                    }
                                    else
                                    {
                                        Helper.Log.Write(Helper.eLogType.Info,
                                            $"[HyperionClient] Fallback selected: {_selectedCaptureMethod.Name}");
                                    }
                                }
                            }
                        }
