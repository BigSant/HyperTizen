...
**Current Status:**
- **Tizen 9 / QE77S95F / 1296.8 hardware test:** EFL screenshot and
  screenmirror work for full UI frames at about 25-30 FPS, but both return
  black for the hardware video plane.
- **Screenmirror callback ABI:** `(mirror_handle, tbm_surface, user_data)`.
- **Integrated fallback:** continuous EFL screenmirror and synchronous EFL
  screenshot are production capture methods. Runtime black/error thresholds
  advance through the retained selector chain.
...
3. Failed methods are cleaned up, but untested fallbacks remain available
4. Black-frame/error thresholds advance to the next viable method at runtime
